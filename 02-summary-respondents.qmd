...
tbl01 |>
  ...
  kbl(col.names=c("label", "short", "count", "%", "valid%", "cum%", 
                  "label", "short", "count", "%", "valid%", "cum%")) |>
  add_header_above(c("Educational level first parent"=6, "Educational level second Parent"=6))|>
  kable_classic()
```
...
The newly created variable `SES` now contains the hightest educational level considering both parents.
...
df.geam01 |>
  select(SDEM017, SDEM017.comp, SDEM018, SDEM018.comp, SES) |>
  head() |>
  kbl(col.names=c("Edu level parent 1 (SDEM017)", 
                  "Score", 
                  "Edu level parent 2 (SDEM018)", 
                  "Score", 
                  "Highest (SES)"))
```

Then it is easy to assign the respondents socioeconomic status to three groups, consisting of "Higher SES", "Medium SES", or "Lower SES"
...
#| echo: false
#| eval: false
#| label: tbl-teaching-wcjc023
#| tbl-cap: "Frequency table of teaching responsibilities - WCJC023"
#| cap-location: margin
...
```{r}
#| echo: false
#| label: tbl-cross-WCJC011
#| fig.cap: "Cross tab of gender and type of contract. Cells indicate absolute numbers"
#| cap-location: margin
...
df.geam02 |>
  ...
  ggally_crosstable(mapping = aes(x = SDEM004, y = WCJC011), cells = c("observed")) +
  labs(x="", y="")
...
Chapter 3 will explore in detail if gender gaps between specific categories are significant or not. 


```{r}
#| echo: false
#| label: tbl-cross-wcjc001
#| fig-cap: "Cross tab of job position (WCJC001) and type of contract (WCJC011)"
#| cap-location: margin

library(GGally)

df.geam02 |>
  filter(!is.na(WCJC001) & !is.na(WCJC011)) |>
  ggally_crosstable(mapping = aes(x = WCJC001, y = WCJC011), cells = c("observed")) +
  labs(x="", y="")
