...
df.geam01 |>
  filter(!is.na(WCJC001) & !is.na(SDEM004.bin)) |>
  ggally_crosstable(mapping = aes(x = WCJC001 , y =SDEM004.bin ), cells = c("observed")) +
  labs(x="", y="")
...
Next, we create a subsample by removing missing values `(NAs)` from `SDEM004.bin` and `WCJC001` and excluding the `Other` category from WCJC001.
...
df.sub.01 <- df.geam01 |>
  ...
  transmute(
    WCJC001 = fct_drop(WCJC001), 
    SDEM004.bin = fct_drop(SDEM004.bin)
  )
```
...
crosstab_report(df.sub.01, "SDEM004.bin", "WCJC001", 
                var1_label = "Gender", var2_label = "Job Position")
...
Now, we will explore whether gender influences the likelihood of holding a fixed-term or permanent contract within the organisation. We will assess whether there is a statistically significant relationship between gender and contract type.
...
Again, we will first clean the database and apply association tests for qualitative variables.
...
crosstab_report(df.sub2, "SDEM004.bin", "WCJC011", 
                var1_label = "Gender", var2_label = "Type of contract")
...
Parental leave policies play a crucial role in promoting work-life balance and gender equality in the workplace. However, access to and uptake of parental leave can vary by gender, potentially reflecting societal norms, workplace expectations, or policy limitations.

The question `WCWI011a` asks about who has taken parental leave.

+-----------+-------------------------------------------------------+----------------------------------------------+
| Code      | Question                                              | Responses                                    |
+===========+=======================================================+==============================================+
| WCWI011a  | Considering the last 5 years, have you taken or are   |                                              |
|           |  you currently taking any form or parental leave?     | \[1\] No\                                    |
|           |                                                       | \[2\] Yes\                                   |
|           |                                                       | \[3\] Prefer not to say                      | 
+-----------+-------------------------------------------------------+----------------------------------------------+
...
```{r}
#| echo: false
#| label: binary-sdem004-gender-geam03
...
We will follow the process (graphic and test) to evaluate whether gender influences whether or not a person takes parental leave.

```{r}
#| echo: false
#| label: fig-WCWI011a-gender
#| fig.cap: "Stacked bar chart of WCWI011a and SDEM004.bin – Parental leave and gender"
#| fig-alt: "Stacked bar chart of WCWI011a and SDEM004.bin – Parental leave and gender"
#| cap-location: margin  
...
ggplot(df.prop.03, aes(x = SDEM004.bin, y = prop, fill = WCWI011a)) +
  geom_col(width = 0.8, position = "stack") +
  geom_text(aes(label = label), 
            position = position_stack(vjust = 0.5), 
            color = "white", size = 3.5, lineheight = 0.9) +
...
```{r}
#| echo: true
#| label: tlb-test-assoc-parental-leave-gender
#| tbl-cap: "Statistical test results - Association between parental leave and gender"
#| cap-location: margin

crosstab_report(df.sub3, "SDEM004.bin", "WCWI011a", 
                var1_label = "Gender", var2_label = "Parental leave")

```

The test result was not statistically significant (χ² = 0.344, p = 0.557 and Fisher’s p = 0.579), indicating that any observed differences could be due to chance rather than a meaningful pattern. Furthermore, the Cramér’s V value (0.029) suggests no association, reinforcing the conclusion that the relationship between gender and parental leave uptake is minimal. 
...
Another aspect where possible gender gaps can be explored is caregiving responsibilities. The GEAM examines this issue in relation to caregiving for a dependent adult (`WCWI006`) or children (`WCWI008`).
...
Once again, we will begin by visually examining the gender distribution of these variables, as this serves as the initial step in identifying potential gaps. We will then proceed to confirm and statistically test our hypothesis.


```{r}
#| echo: false
#| label:  fig-care-resp-WCWI006-gender-geam03
#| fig.cap: "Stacked bar chart of WCWI006 and SDEM004.bin – Primary Caregiving for Dependent Adults by Gender"
#| fig-alt: "Stacked bar chart of WCWI006 and SDEM004.bin – Primary Caregiving for Dependent Adults by Gender"
#| cap-location: margin

cpal <- RColorBrewer::brewer.pal(7, "Set1")

#  Calculate gender proportions

df.prop.03 <- df.geam03 |> 
  filter(!is.na(WCWI006) & !is.na(SDEM004.bin)) |> 
  group_by(SDEM004.bin, WCWI006) |> 
  summarise(n = n(), .groups = "drop") |> 
  group_by(SDEM004.bin) |> 
  mutate(
    prop = n / sum(n),
...
ggplot(df.prop.03, aes(x = SDEM004.bin, y = prop, fill = WCWI006)) +
  geom_col(width = 0.8, position = "stack") +
  geom_text(aes(label = label), 
            position = position_stack(vjust = 0.5), 
            color = "white", size = 3.5, lineheight = 0.9) +
  scale_fill_manual(values = cpal) +
  scale_y_continuous(labels = percent_format()) +
  labs(x = "", y = "Proportion", fill = "Primary Caregiving for Dependent Adults") +
  theme_light()
...
The chart depicts the distribution of responses regarding whether individuals serve as the primary caregiver for dependent adults, disaggregated by gender. Among men, 10% reported having this responsibility, 85% indicated they do not, and 6% preferred not to answer. In comparison, 20% of women reported being primary caregivers, 76% stated they are not, and 4% selected ‘Prefer not to say’. The data reveal a higher proportion of women reporting caregiving responsibilities for dependent adults relative to men. While the majority in both groups reported not assuming such responsibilities, the gender gap in affirmative responses suggests differences in caregiving roles that may warrant further investigation. Statistical analysis would be required to determine whether the observed difference is statistically significant.


```{r}
#| echo: false
#| label:  fig-care-resp-WCWI008-gender-geam03
#| fig.cap: "Stacked bar chart of WCWI008 and SDEM004.bin – - Parental/Legal guardianship of children by gender"
#| fig-alt: "Stacked bar chart of WCWI008 and SDEM004.bin – - Parental/Legal guardianship of children by gender"
#| cap-location: margin
...
ggplot(df.prop.03, aes(x = SDEM004.bin, y = prop, fill = WCWI008)) +
  geom_col(width = 0.8, position = "stack") +
  geom_text(aes(label = label), 
            position = position_stack(vjust = 0.5), 
            color = "white", size = 3.5, lineheight = 0.9) +
  scale_fill_manual(values = cpal) +
  scale_y_continuous(labels = percent_format()) +
  labs(x = "", y = "Proportion", fill = "Parental/Legal guardianship of children") +
  theme_light()


```

The chart shows the distribution of responses concerning parental or legal guardianship of children, broken down by gender. Among men, 31% reported having parental or legal guardianship responsibilities, 68% indicated they do not, and 1% preferred not to respond. Similarly, among women, 32% answered ‘Yes’, 67% answered ‘No’, and 1% selected ‘Prefer not to say’. The results reveal a nearly identical pattern across genders, with only a marginal difference in reported guardianship responsibilities. These findings suggest that, within the sample, the distribution of parental or legal guardianship roles is broadly balanced between men and women. Nonetheless, further statistical testing would be necessary to confirm whether this apparent similarity is statistically non-significant.
...
Now, we apply the association tests

```{r}
#| echo: false 
#| label: sample-care-resp-gender

df.sub3 <- df.geam03 |>
  filter(!is.na(CareResp) & CareResp != "Prefer not to say" & !is.na(SDEM004.bin)) |>
  transmute(
    CareResp = fct_drop(as.factor(CareResp)), 
    SDEM004.bin = fct_drop(SDEM004.bin)
  )
...
```{r}
#| echo: true
#| label: tlb-test-assoc-care-resp-gender
#| tbl-cap: "Statistical test results - Association between parental leave and gender"
#| cap-location: margin

crosstab_report(df.sub3, "SDEM004.bin", "CareResp", 
                var1_label = "Gender", var2_label = "Care Responsibilities")

```

The table displays the distribution of care responsibilities by gender. Among men, 36.6% reported having care responsibilities, compared to 46.7% of women. The chi-squared test of independence indicated that this difference is statistically significant (χ² = 4.85, df = 1, p = 0.0276), suggesting an association between gender and caregiving roles. The strength of the association, as measured by Cramér’s V, was 0.092, indicating a weak effect size. 

These results suggest that women in the dataset are significantly more likely to report having care responsibilities compared to men. While the association is weak, the findings point towards a gendered distribution of care responsibilities, which may warrant further investigation into underlying factors influencing this disparity.
...
df.geam01 <- df.geam01 |>  
  mutate(across(starts_with("WCWI005.SQ"), as.numeric),  # Convert selected columns to numeric
         FlexMean = rowMeans(across(starts_with("WCWI005.SQ")), na.rm = TRUE))  # Compute mean directly
```
...
ggplot(df.sub.01, aes(x = SDEM004.bin, y = FlexMean, fill = SDEM004.bin)) + 
...
::: callout-note
The independent samples t-test assumes: `(1)` independence of observations, meaning that the data from one participant do not influence another; `(2)` normality of the dependent variable within each group, which can be assessed using the Shapiro–Wilk test or visual inspection; and `(3)` homogeneity of variances between groups, typically tested with Levene’s test. If normality or equal variances are violated, alternative approaches such as Welch’s t-test or non-parametric methods like the Mann–Whitney U test may be more appropriate.
:::
...
```{r}
#| echo: true
#| label: check-assumptions-mean-comparison-variance
#| tbl-cap: "Levene test for Homogeneity of Variances"

library(car)      

leveneTest(FlexMean ~ SDEM004.bin, data = df.sub.01)
```
...
```{r}
#| label: boxplot-job-satisfaction-gender-education-level
...
```{r}
#| echo: true
#| label: check-assumptions-anova-normality-graph
...
```{r}
#| echo: true
#| label: check-assumptions-anova-normality-test
#| tbl-cap: "Shapiro-Wilk Test for Normality"

# Check normality assumption by groups
df.sub.03 %>%
  group_by(SDEM004.bin, educ_level) %>%
  shapiro_test(JobSat_score)
...
```{r}
#| echo: true
#| label: check-assumptions-anova-variance-homogeneity
#| tbl-cap: "Levene test for Homogeneity of Variances"

df.sub.03 %>% levene_test(JobSat_score ~ SDEM004.bin*educ_level)
