...
Since these stacked barchars are quite frequent, we have created a custom function `crosstab_plot()` to generate the plots on the fly with different variables. It saves us from repeating the same lines of code for each plot. 
:::
...
df.sub.03 |>
  ...
  summarize(
    N = n(),
    Mean_Flex = mean(JobSF.score, na.rm = TRUE),
    SD_Flex = sd(JobSF.score, na.rm = TRUE),
